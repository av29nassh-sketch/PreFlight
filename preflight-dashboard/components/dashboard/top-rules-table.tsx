...
export function TopRulesTable({ rows }: TopRulesTableProps) {
  return (
    <section className="rounded-lg border border-zinc-800 bg-zinc-900/70 shadow-[0_18px_80px_rgba(0,0,0,0.26)] backdrop-blur-sm">
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[560px] text-left text-sm">
          <thead className="bg-zinc-950/70 font-mono text-xs uppercase tracking-[0.12em] text-zinc-500">
            <tr>
          ...
          <tbody className="divide-y divide-zinc-800">
            {rows.length === 0 ? (
              ...
              rows.map((row) => (
                <tr key={row.ruleId} className="bg-zinc-950/55">
                  <td className="px-5 py-4">
                    <code className="rounded-md border border-cyan-300/20 bg-cyan-300/10 px-2 py-1 text-xs text-cyan-100">
                      {row.ruleId}
                    </code>
                  </td>
